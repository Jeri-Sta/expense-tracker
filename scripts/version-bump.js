...
function updateChangelog(newVersion, changelogContent) {
  ...
  // Remove apenas linhas template
  const cleanedContent = devSectionFull
    .replace(/- Adicione .* aqui/g, '')
    .replace(/## \[Em Desenvolvimento\]/, '')
    .replace(/---/g, '')
    .trim();

  const finalVersionContent = cleanedContent || '*Nenhuma mudança registrada.*';
...
`.trim() + '\n';
  ...
  logSuccess('CHANGELOG atualizado mantendo estrutura por categoria');
}
